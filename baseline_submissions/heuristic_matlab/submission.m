...
files = dir([TEST_DATA_DIR , '*.csv']);
for i = 1:length(files)
    datalist = [datalist; fullfile(TEST_DATA_DIR, files(i).name)];
end
...
%%
prediction_all = [];
for idx_data = 1:length(datalist)
    ...
    satcat_temp = split(data_path,["/","."]);
    satcat = satcat_temp{end-1};
    ...
    for i = 1:length(nodes)
        ObjectID_data = [ObjectID_data; nodes{i}.satcat];
        TimeIndex_data = [TimeIndex_data; hours(nodes{i}.t - starttime) / 2 ];
        Direction_data = [Direction_data; nodes{i}.signal];
        Node_data = [Node_data; nodes{i}.type];
        Type_data = [Type_data; nodes{i}.mtype];
    end

    %% Create a table
    prediction = table(ObjectID_data, TimeIndex_data, Direction_data, Node_data, Type_data);
    prediction.Properties.VariableNames = ["ObjectID","TimeIndex","Direction","Node","Type"];
    prediction = sortrows(prediction,'TimeIndex');
    % Display the table

    prediction_all = [prediction_all;prediction];
end

%%
% Save the prediction table to a CSV file
writetable(prediction_all, TEST_PREDS_FP);
disp('Prediction saved.')